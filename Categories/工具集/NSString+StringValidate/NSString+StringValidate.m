...
+ (NSAttributedString *)attributedStringWithStrings:(NSArray<NSString *> *)strings fonts:(NSArray<UIFont *> *)fonts colors:(NSArray<UIColor *> *)colors {
    return [self attributedStringWithStrings:strings fonts:fonts colors:colors spacingRow:0 alignment:nil];
}
...
+ (NSAttributedString *)attributedStringWithStrings:(NSArray<NSString *> *)strings fonts:(NSArray<UIFont *> *)fonts colors:(NSArray<UIColor *> *)colors spacingRow:(CGFloat)spacing alignment:(NSString *)alignment {
    ...
    if (spacing != 0) {
        ...
        [firstPart addAttribute:NSParagraphStyleAttributeName value:paragraph range:NSMakeRange(0, firstPart.length)];
    }
    return firstPart;
}
    /// 添加行间距
- (NSAttributedString *)attributedStringSpacingRow:(CGFloat)spacing {
    NSMutableAttributedString *firstPart = [[NSMutableAttributedString alloc] initWithString:self];
    NSMutableParagraphStyle *paragraph = [[NSMutableParagraphStyle alloc] init];
    paragraph.lineSpacing = spacing;
    [firstPart addAttribute:NSParagraphStyleAttributeName value:paragraph range:NSMakeRange(0, firstPart.length)];
    return firstPart;
...
+ (NSAttributedString *)attributedStringWithStrings:(NSArray<NSString *> *)strings fonts:(NSArray<UIFont *> *)fonts colors:(NSArray<UIColor *> *)colors links:(NSArray<NSNumber *> *)links {
    ...
    if (strings && strings.count > 0) {
        [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NSMutableAttributedString * otherPart = [[NSMutableAttributedString alloc] initWithString:obj];
            NSMutableDictionary * otherAttributes = @{NSFontAttributeName:fonts[idx],NSForegroundColorAttributeName:colors[idx]}.mutableCopy;
            for (NSNumber *num in links) {
                if ([num integerValue] == idx) {
                    otherAttributes[NSUnderlineStyleAttributeName] = [NSNumber numberWithInteger:NSUnderlineStyleSingle];
                    break;
                }
            }
            [otherPart setAttributes:otherAttributes range:NSMakeRange(0,otherPart.length)];
...
+ (CGFloat)achiveWidthAttrString:(NSAttributedString *)attrString withHeight:(CGFloat)height {
    CGRect desRect = [attrString boundingRectWithSize:CGSizeMake(MAXFLOAT, height) options:NSStringDrawingUsesLineFragmentOrigin context:nil];
    return desRect.size.width;
}
