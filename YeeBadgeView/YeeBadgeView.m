...
static UIImage  *_yeeRedDotImage(CGFloat redDotRadius,UIColor *redDotColor, UIColor *redDotBordeColor, CGFloat redDotBordeWidth, CGSize lableSize){
    if (lableSize.width+5<lableSize.height) {
        lableSize.width = lableSize.height-5.;
    } UIGraphicsBeginImageContextWithOptions(CGSizeMake(lableSize.width+5,lableSize.height),NO, [UIScreen mainScreen].scale);
    
    UIBezierPath* roundedRectanglePath = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(redDotBordeWidth, redDotBordeWidth, lableSize.width+5-redDotBordeWidth*2, lableSize.height-redDotBordeWidth*2) cornerRadius:redDotRadius-redDotBordeWidth];
    [redDotColor setFill];
    [roundedRectanglePath fill];
    [redDotBordeColor setStroke];
    [roundedRectanglePath stroke];
    roundedRectanglePath.lineWidth = redDotBordeWidth;
    UIImage *image= UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    return image;
}
...
-(void)update_subViews{
    ...
    switch (_redDotStytle) {
        ...
        default:
        {
            ...
            self.m_pBadgeImageView.image = _yeeRedDotImage(self.m_pBadgeLable.frame.size.height, _redDotColor, _redDotBoderColor, _redDotBorderWith, self.m_pBadgeLable.frame.size);
        }
        break;
    }
}
...
-(void)setRedDotNumber:(NSInteger)redDotNumber{
    ...
    self.m_pBadgeLable.text =[NSString stringWithFormat:@"%@",redDotNumber>_redDotMaxNumber?[NSString stringWithFormat:@"%@+", @(redDotNumber)]:@(redDotNumber)];
    [self update_subViews];
}
...
- (void)setRedDotBoderColor:(UIColor *)redDotBoderColor {
    if (redDotBoderColor==nil) {
        redDotBoderColor = [UIColor clearColor];
    }
